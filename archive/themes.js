// Theme system for Charlie News.
// Three aesthetic directions × accent hue × light/dark, resolved into flat tokens.
// Accents all share chroma+lightness, varying only hue, so they stay harmonious.
...
const THEMES = {
  editorial: {
    ...
    headlineLine: 1.18,
    labelStyle: 'serif-italic',   // how the source/topic labels render
    card: 'paper',                 // separated by hairlines on paper
    radius: 16,
  ...
  minimal: {
    ...
    card: 'list',                  // hairline-divided list, no fills
    radius: 4,
  ...
  warm: {
    ...
    card: 'soft',                  // rounded cards with soft shadow
    radius: 22,
...
// Accent options as a flat list (light swatch is the stored tweak value).
window.ACCENT_OPTIONS = Object.keys(window.ACCENTS).map((name) => ({
  name, light: window.ACCENTS[name].light, dark: window.ACCENTS[name].dark,
}));
window.resolveAccent = function (lightStr, dark) {
  const opt = window.ACCENT_OPTIONS.find((o) => o.light === lightStr) || window.ACCENT_OPTIONS[0];
  return dark ? opt.dark : opt.light;
};

window.buildTheme = function (themeId, accentResolved, dark) {
  const t = THEMES[themeId] || THEMES.editorial;
  const mode = dark ? t.dark : t.light;
  const acc = accentResolved || window.ACCENTS.red[dark ? 'dark' : 'light'];
  return {
    id: themeId,
    label: t.label,
